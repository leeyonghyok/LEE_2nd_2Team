...
def preprocess_pipeline(file_path=None):
    # ==========================================
    # 1. 데이터 불러오기
    # ==========================================
    print("1. 데이터셋 로드 중...")
    ...
    pd.Series(y_train, name='Churn').to_csv(os.path.join(save_dir, 'y_train.csv'), index=False)
    pd.Series(y_val,   name='Churn').to_csv(os.path.join(save_dir, 'y_val.csv'),   index=False)
    pd.Series(y_test,  name='Churn').to_csv(os.path.join(save_dir, 'y_test.csv'),  index=False)
